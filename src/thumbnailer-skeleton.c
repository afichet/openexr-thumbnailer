...
static char *get_target_uri(GFile *file)
{
    ...
    target = g_strdup(g_file_info_get_attribute_string(
        info,
        G_FILE_ATTRIBUTE_STANDARD_TARGET_URI));
    g_object_unref(info);
...
#    if !GDK_PIXBUF_CHECK_VERSION(2, 36, 5)
            scaled = gnome_desktop_thumbnail_scale_down_pixbuf(
                pixbuf,
...
#else
#    error                                                                     \
        "One of THUMBNAILER_RETURNS_PIXBUF or THUMBNAILER_RETURNS_DATA must be set"
#endif
    g_free(input_filename);

    if (!pixbuf) {
        g_warning(
            "Could not thumbnail '%s': %s",
            filenames[0],
            error ? error->message
                  : "Thumbnailer failed without returning an error");
        g_clear_error(&error);
    ...
    if (gdk_pixbuf_save(pixbuf, output, "png", &error, NULL) == FALSE) {
        g_warning(
            "Couldn't save the thumbnail '%s' for file '%s': %s",
            output,
            filenames[0],
            error->message);
        g_error_free(error);
